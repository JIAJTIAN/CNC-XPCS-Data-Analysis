...
DoseNum = 6;
Doselim = 0.1;% this is what is was previouslyDoseArrayTota(DoseNum);
Threshold = 0.2;
RefinedLength = 6;
EqDiff = 0.05;
RpDiff = 0.05;
...
CelluloseType = 'CNC';
Solvent = 'PG';
PlotCell{1} = [9, 12, 17, 22, 32]; % concentration variation
PlotCell{2} = [23, 24, 25]; % MagneticfieldExposureTime Variation
PlotCell{3} = [35, 3, 6]; % CaCl2 variation
    ...
    for CaseNum = 1:3 % determiens what variable (concentration, time, or CaCl2) is changed
        ...
        switch CaseNum
            ...
            case 2
                ...
                Xarray = [0 3 28]; % don't see a 1 hour one, there was 1 hour in Xarray in the original
                XUnit = 'Time(h)';
            case 3
        ...
        for qNum = 1:8 % go through all of the q values
            for PlotIndex = 1:length(Xarray) % this determines what folders to go into for this case 
                ...
                for SacnIndex = 1:RunIndex % RunIndex = 6
                    ScanNameCell{SacnIndex} = SaveNameCell{DoseIndex(SacnIndex),ExpNumIndex}; % creates new name cell with only certain cells
                end
...
%                
%                 %% Data Fitting
%                 FitResult = StrExpFit(TAU, C);
%         
%                 CFitKWW = FitResult{1};
%                 CoefArrayKWW = FitResult{2};
%                 GammaKWW(qNum) = CoefArrayKWW(1);
%                 littleGamma(qNum) = CoefArrayKWW(2); % records little Gamma
%         
% 
%                 CFitSingle = FitResult{4};
%                 CoefArraySingle = FitResult{5};
%                 GammaSingle(qNum) = CoefArraySingle(1);
% 
%                 qArray(qNum) = qValue;
%                 PlotTAU{qNum} = TAU;
%                 PlotG2{qNum} = C;
%                 PlotG2FitKWW{qNum} = CFitKWW;
%                 PlotG2FitSingle{qNum} = CFitSingle;
                
                
                
                    % I don't think the rest of this is old stuff
